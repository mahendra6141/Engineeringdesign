...
function movementCalculation()
{
    ...
    let xExt = parseFloat(document.getElementById("xExtInput").value);
	...
	let presetComp = parseFloat(document.getElementById("presetComp").value);
    ...
    document.getElementById("xExt").innerHTML = xExt.toFixed(3);
    ...
    let KthetalComp =1 + (0.0094 * y1 * Math.pow((Lb / Dm),1.33));	
...
let KthetalExt = 1 + (0.0094 * presetY * Math.pow((Lb / Dm),1.33));
...
let KrExt =(2 * (q + exExt) + rotationEffectExt + eyExt) / (2 * q);
   ...
   let Kr = Math.max( KrComp, KrExt, 1);
