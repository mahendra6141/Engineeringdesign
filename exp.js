...
function movementCalculation()
{
    ...
    let exComp = xComp / N;
    let eyComp = (3 * Dm * y1) / (N * (Lb - xComp));
    let ethetaComp = (theta * Math.PI / 180) * Dm / (2 * N);
         KthetalComp =1 + (0.0094 * y1 * Math.pow((Lb / Dm),1.33));	
...
// Preset Condition
//===========================
let exExt = presetExt / N;
let eyExt = (3 * Dm * presetY) / (N * (Lb + presetExt));
let ethetaExt = (presetTheta * Math.PI / 180) * Dm / (2 * N);
    KthetalExt = 1 + (0.0094 * presetY * Math.pow((Lb / Dm),1.33));
...
else
{
    ...
    Dm*
    KthetalExt*
    Math.sin((presetTheta*Math.PI/180)/2)*
    (Lb+presetExt));
}
    ...
    let Ro = Math.min(RoComp,RoExt);

    let Ctheta = Math.min(Ro,1);
...
let Psc =(0.34 * Math.PI * Ctheta * fiu) / (Math.pow(N,2) * q);

    //=========================
    // Display
    //=========================

    document.getElementById("RoComp").innerHTML = RoComp.toFixed(3);

    document.getElementById("RoExt").innerHTML = RoExt.toFixed(3);

    document.getElementById("RoFinal").innerHTML = Ro.toFixed(3);
...
function springRateCalculation()
{
    ...
    document.getElementById("fwResult").innerHTML = fw.toFixed(3);

    document.getElementById("KsrResult").innerHTML = Ksr.toFixed(3);
