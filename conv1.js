function openTab(evt, tabName) {
    ...
    // JavaScript se tab open hua
        ...
        for (let i = 0; i < buttons.length; i++) {
            ...
            let onclickText = buttons[i].getAttribute("onclick");

            if (
                onclickText &&
                onclickText.includes("'" + tabName + "'")
            ) {
...
function updateIdlerWeight(force = false){
    ...
    if(carryData){
        ...
        if(force || document.getElementById("carryWeight").value===""){

            document.getElementById("carryWeight").value =
            carryData.carry;
    ...
    if(returnData){
        ...
        if(force || document.getElementById("returnWeight").value===""){

            document.getElementById("returnWeight").value =
            returnData.return;
...
let mat=this.value.trim();

if(materialData[mat]!=undefined){

document.getElementById("rho").value=
materialData[mat].toFixed(2);
...
function updateBeltData(){
    ...
    // Belt thickness sirf belt change hone par update hogi
...
function updateSlopeFactor(){

    let angle = +document.getElementById("cs").value;

    let angles = Object.keys(slopeFactorTable)
        .map(Number)
        .sort((a,b)=>a-b);
    ...
    if(angle <= angles[0]){
        document.getElementById("K").value =
            slopeFactorTable[angles[0]].toFixed(2);
        return;
    }
    ...
    if(angle >= angles[angles.length-1]){
        document.getElementById("K").value =
            slopeFactorTable[angles[angles.length-1]].toFixed(2);
        return;
    }
...
let mc =
+document.getElementById("carryWeight").value / 9.81;

let mi = mc;

let mr_w =
+document.getElementById("returnWeight").value / 9.81;
...
function calcPower(){
    ...
    let manufacturerMotor =
        document.getElementById("selectedMotorPower");

    if(selectedMotor){

        // Har calculation par manufacturer value ko recommended ke equal set karo
        manufacturerMotor.value = selectedMotor;
...
//////////////Tilted Idler input//////
/////////////////////////////////////
function toggleTiltedIdler(){

    const yes =
        document.querySelector(
            'input[name="tiltedIdler"]:checked'
        ).value === "Yes";

    const angle = document.getElementById("i");
    const length = document.getElementById("L1");
    ...
    if(yes){
        ...
        if(angle.value == "0") angle.value = 2;
...
function loadProject(){
    ...
    const selected = document.getElementById("projectList").value;
...
function printReport(){
    ...
    generateReport();   // latest values fill karega

    window.print();     // phir print dialog kholega
}
...
copyValue("carryWeight","carryWeightc");
copyValue("returnWeight","returnWeightc");
...
function calculateDriveComponents(){
    ...
    showMessage("Drive Components Calculated");
    openTab(null,"BeltForces");
